...
You can change some runtime behaviour of Chainsaw by setting a configuration
value in Chainsaws config module. These can be persisted to hard disk to be
...
>>> from chainsaw import config # doctest: +SKIP
...
To create an editable configuration file, use the :py:func:`chainsaw.config.save` method:

>>> from chainsaw import config # doctest: +SKIP
>>> config.save('/tmp/chainsaw_current.cfg') # doctest: +SKIP

This will store the current runtime configuration values in the given file.
Note that these settings will not be used on the next start of chainsaw, because
you first need to tell us, where you have stored this file. To do so, please
set the environment variable **"CHAINSAW_CFG_DIR"** to the directory, where you have
...
--------------------------
Default settings are stored in a provided chainsaw.cfg file, which is included in
the Python package:

.. literalinclude:: ../../chainsaw/chainsaw.cfg
    :language: ini
...
its final set of settings. It searches for the file 'chainsaw.cfg' in several
locations with different priorities:

#. $CWD/chainsaw.cfg
#. $HOME/.chainsaw/chainsaw.cfg
#. ~/chainsaw.cfg
#. $PYTHONPATH/chainsaw/chainsaw.cfg (always taken as default configuration file)

Note that you can also override the location of the configuration directory by
setting an environment variable named **"chainsaw_CFG_DIR"** to a writeable path to
...
>>> from chainsaw import config # doctest: +SKIP
>>> config.load('chainsaw_silent.cfg') # doctest: +SKIP

    """
    DEFAULT_CONFIG_DIR = os.path.join(os.path.expanduser('~'), '.chainsaw')
    DEFAULT_CONFIG_FILE_NAME = 'chainsaw.cfg'
    DEFAULT_LOGGING_FILE_NAME = 'logging.yml'

    __name__ = 'chainsaw.util.config'
    __file__ = __file__
    ...
    def default_config_file(self):
        """ default config file living in chainsaw package """
        return pkg_resources.resource_filename('chainsaw', os.path.join('_resources', Wrapper.DEFAULT_CONFIG_FILE_NAME))

    @property
    def default_logging_file(self):
        return pkg_resources.resource_filename('chainsaw', os.path.join('_resources', Wrapper.DEFAULT_LOGGING_FILE_NAME))

    def keys(self):
        return self._conf_values.options('chainsaw')

    @property
    def cfg_dir(self):
        """ chainsaws configuration directory (eg. ~/.chainsaw)"""
        return self._cfg_dir

    @cfg_dir.setter
    def cfg_dir(self, chainsaw_cfg_dir):
        """ Sets chainsaws configuration directory.
        Also creates it with some default files, if does not exists. """
        if not os.path.exists(chainsaw_cfg_dir):
            try:
                mkdir_p(chainsaw_cfg_dir)
            except EnvironmentError:
                raise ConfigDirectoryException("could not create configuration directory '%s'" % chainsaw_cfg_dir)
            except NotADirectoryError:  # on Python 3
                raise ConfigDirectoryException("chainsaw cfg dir (%s) is not a directory" % chainsaw_cfg_dir)

        if not os.path.isdir(chainsaw_cfg_dir):
            raise ConfigDirectoryException("%s is no valid directory" % chainsaw_cfg_dir)
        if not os.access(chainsaw_cfg_dir, os.W_OK):
            raise ConfigDirectoryException("%s is not writeable" % chainsaw_cfg_dir)

        # give user the default cfg file, if its not there
        self.__copy_default_files_to_cfg_dir(chainsaw_cfg_dir)
        self._cfg_dir = chainsaw_cfg_dir

        if self.show_config_notification:
            stars = '*' * 80
            print(stars, '\n',
                  'Changed chainsaws config directory to "{dir}".\n'
                  'To make this change permanent, export the environment variable'
                  ' "chainsaw_CFG_DIR" \nto point to this location. Eg. edit your .bashrc file!'
                  .format(dir=chainsaw_cfg_dir), '\n', stars, sep='')
    ...
    def cache_dir(self):
        ...
        value = self._conf_values.get('chainsaw', 'cache_dir')
    ...
    def cache_dir(self, value):
        ...
        self._conf_values.set('chainsaw', 'cache_dir', value)

    @property
    def logging_config(self):
        cfg = self._conf_values.get('chainsaw', 'logging_config')
        if cfg == 'DEFAULT':
    ...
    #    """ Try to re-initialize logging system for package 'chainsaw'.
    #
    #    Parameters
    #    ----------
    #    config: dict
    ...
    #    """
    #    from chainsaw.util.log import setup_logging
    #   #config['incremental'] = True
    #    setup_logging(self, config)

    @property
    def traj_info_max_entries(self):
        return self._conf_values.getint('chainsaw', 'traj_info_max_entries')

    @traj_info_max_entries.setter
    def traj_info_max_entries(self, val):
        self._conf_values.set('chainsaw', 'traj_info_max_entries', str(val))

    @property
    def traj_info_max_size(self):
        return self._conf_values.getint('chainsaw', 'traj_info_max_size')

    @traj_info_max_size.setter
    def traj_info_max_size(self, val):
        val = str(int(val))
        self._conf_values.set('chainsaw', 'traj_info_max_size', val)

    @property
    def show_progress_bars(self):
        return self._conf_values.getboolean('chainsaw', 'show_progress_bars')

    @show_progress_bars.setter
    def show_progress_bars(self, val):
        self._conf_values.set('chainsaw', 'show_progress_bars', str(val))

    @property
    def use_trajectory_lengths_cache(self):
        return self._conf_values.getboolean('chainsaw', 'use_trajectory_lengths_cache')

    @use_trajectory_lengths_cache.setter
    def use_trajectory_lengths_cache(self, val):
        self._conf_values.set('chainsaw', 'use_trajectory_lengths_cache', str(val))

    @property
    def show_config_notification(self):
        return self._conf_values.getboolean('chainsaw', 'show_config_notification')

    @show_config_notification.setter
    def show_config_notification(self, val):
        self._conf_values.set('chainsaw', 'show_config_notification', str(val))
    ...
    def __getitem__(self, name):
        try:
            return self._conf_values.get('chainsaw', name)
        except KeyError:  # re-try with default section
            return self._conf_values.get(name)

    def __setitem__(self, name, value):
        value = str(value)
        self._conf_values.set('chainsaw', name, value)

    @staticmethod
    def _format_msg(msg):
        from chainsaw import __version__
        return "[chainsaw {version}] {msg}".format(version=__version__, msg=msg)

    def __repr__(self):
        cfg_pairs = {k: getattr(self, k) for k in self.keys()}
        return "[chainsaw Config: {}]".format(cfg_pairs)
